...
impl Ease {
    pub fn apply(self, t: f32) -> f32 {
        ...
        match self {
            ...
            Ease::BackIn => {
                const C1: f32 = 1.70158; const C3: f32 = C1 + 1.0;
                C3 * t * t * t - C1 * t * t
            }
            Ease::BackOut => {
                const C1: f32 = 1.70158; const C3: f32 = C1 + 1.0;
...
impl<T: Lerp> Tween<T> {
    pub fn new(start: T, end: T, duration: f32) -> Self {
        Self { start, end, duration: duration.max(0.0), ease: Ease::Linear, looping: TweenLoop::Once, elapsed: 0.0 }
    }

    pub fn with_ease(mut self, ease: Ease) -> Self { self.ease = ease; self }
    pub fn with_loop(mut self, looping: TweenLoop) -> Self { self.looping = looping; self }

    pub fn update(&mut self, dt: f32) -> T {
        if self.duration <= 0.0 { return self.end; }
        self.elapsed += dt;
        match self.looping {
            TweenLoop::Once => { self.elapsed = self.elapsed.min(self.duration); }
            TweenLoop::Loop => { self.elapsed = self.elapsed.rem_euclid(self.duration); }
            TweenLoop::PingPong => {}
        }
        self.value()
    }
    ...
    pub fn progress(&self) -> f32 { self.raw_t() }
    ...
    pub fn seek(&mut self, t: f32) { self.elapsed = t.clamp(0.0, 1.0) * self.duration; }
    ...
    fn raw_t(&self) -> f32 {
        ...
        match self.looping {
            ...
            TweenLoop::PingPong => {
                ...
                if cycle < self.duration { cycle / self.duration }
                else { (period - cycle) / self.duration }
            }
        }
    }
}
...
impl<T: Lerp + Default> Default for Tween<T> {
    fn default() -> Self { Self::new(T::default(), T::default(), 1.0) }
}
