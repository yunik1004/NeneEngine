...
use super::{ColorVertex, MeshVertex};

// ── Helpers ───────────────────────────────────────────────────────────────────

fn cv(x: f32, y: f32, color: [f32; 4]) -> ColorVertex {
    ColorVertex {
        pos: [x, y, 0.0],
        color,
    }
...
fn cv3(x: f32, y: f32, z: f32, color: [f32; 4]) -> ColorVertex {
    ColorVertex {
        pos: [x, y, z],
        color,
    }
...
pub fn rect(x: f32, y: f32, w: f32, h: f32, color: [f32; 4]) -> Vec<ColorVertex> {
...
pub fn circle(cx: f32, cy: f32, radius: f32, color: [f32; 4]) -> Vec<ColorVertex> {
...
pub fn circle_segments(
    ...
    for i in 0..n {
        ...
        out.push(cv(cx, cy, color));
        out.push(cv(cx + a0.cos() * radius, cy + a0.sin() * radius, color));
        out.push(cv(cx + a1.cos() * radius, cy + a1.sin() * radius, color));
    }
    out
}

/// Filled triangle.
pub fn triangle(a: [f32; 2], b: [f32; 2], c: [f32; 2], color: [f32; 4]) -> Vec<ColorVertex> {
    vec![
        cv(a[0], a[1], color),
        cv(b[0], b[1], color),
        cv(c[0], c[1], color),
...
pub fn line(
    ...
    vec![
        cv(x1 + nx, y1 + ny, color),
        cv(x2 + nx, y2 + ny, color),
        cv(x2 - nx, y2 - ny, color),
        cv(x1 + nx, y1 + ny, color),
        cv(x2 - nx, y2 - ny, color),
        cv(x1 - nx, y1 - ny, color),
...
impl QuadBuilder {
    pub fn color(self, color: [f32; 4]) -> Vec<ColorVertex> {
        let (hw, hh) = (self.w * 0.5, self.h * 0.5);
        rect(-hw, -hh, self.w, self.h, color)
    }

    pub fn mesh(self) -> (Vec<MeshVertex>, Vec<u32>) {
        let (hw, hh) = (self.w * 0.5, self.h * 0.5);
        let verts = vec![
            MeshVertex {
                position: [-hw, -hh, 0.0],
...
impl CubeBuilder {
    pub fn color(self, color: [f32; 4]) -> Vec<ColorVertex> {
        ...
        tris.iter()
            .flat_map(|tri| tri.iter().map(|&[x, y, z]| cv3(x, y, z, color)))
            .collect()
    }
    ...
    pub fn color(self, color: [f32; 4]) -> Vec<ColorVertex> {
        ...
        for i in 0..stacks {
            for j in 0..slices {
                ...
                out.push(cv3(ax, ay, az, color));
                out.push(cv3(cx, cy, cz, color));
                out.push(cv3(bx, by, bz, color));
                out.push(cv3(bx, by, bz, color));
                out.push(cv3(cx, cy, cz, color));
                out.push(cv3(dx, dy, dz, color));
            }
        }
        out
    }
    ...
    pub fn mesh(self) -> (Vec<MeshVertex>, Vec<u32>) {
        ...
        for i in 0..=stacks {
            ...
            for j in 0..=slices {
                ...
                let nz = phi.sin() * theta.sin();
                verts.push(MeshVertex {
                    position: [nx * self.radius, ny * self.radius, nz * self.radius],
                    normal: [nx, ny, nz],
...
impl CylinderBuilder {
    pub fn color(self, color: [f32; 4]) -> Vec<ColorVertex> {
        ...
        for i in 0..slices {
            ...
            out.push(cv3(x0, -hh, z0, color));
            out.push(cv3(x1, -hh, z1, color));
            out.push(cv3(x1, hh, z1, color));
            out.push(cv3(x0, -hh, z0, color));
            out.push(cv3(x1, hh, z1, color));
            out.push(cv3(x0, hh, z0, color));
        ...
        for (y, flip) in [(-hh, true), (hh, false)] {
            for i in 0..slices {
                ...
                let (x1, z1) = (a1.cos() * self.radius, a1.sin() * self.radius);
                if flip {
                    out.push(cv3(0.0, y, 0.0, color));
                    out.push(cv3(x1, y, z1, color));
                    out.push(cv3(x0, y, z0, color));
                } else {
                    out.push(cv3(0.0, y, 0.0, color));
                    out.push(cv3(x0, y, z0, color));
                    out.push(cv3(x1, y, z1, color));
    ...
    pub fn mesh(self) -> (Vec<MeshVertex>, Vec<u32>) {
        ...
        for i in 0..=slices {
            ...
            verts.push(MeshVertex {
                position: [nx * self.radius, -hh, nz * self.radius],
                normal: [nx, 0.0, nz],
                uv: [u, 1.0],
            });
            verts.push(MeshVertex {
        ...
        for (y, ny, flip) in [(-hh, -1.0f32, true), (hh, 1.0f32, false)] {
            ...
            for i in 0..slices {
                ...
                verts.push(MeshVertex {
                    position: [cx * self.radius, y, cz * self.radius],
                    normal: [0.0, ny, 0.0],
