...
//! // let renderer = TileMapRenderer::new(ctx, &tileset, &map, tile_world_size);
//!
//! // In update:
//! // renderer.prepare(ctx, &camera, aspect);
//!
//! // In render:
//! // renderer.render(pass, &tileset.texture);
//! ```
...
/// GPU renderer for a [`TileMap`].
///
/// One `TileMapRenderer` is created per map. Each visible [`TileLayer`] is
/// uploaded as a separate batched quad mesh and drawn with a single draw call.
pub struct TileMapRenderer {
    pipeline: Pipeline,
    uniform: UniformBuffer,
    /// Per-layer GPU buffers (vbuf, ibuf, index_count).
    layers: Vec<LayerBuffers>,
    /// World-space size of one tile (square).
    pub tile_size: f32,
}
...
impl TileMapRenderer {
    /// Build GPU buffers for all layers of `map` using tiles from `tileset`.
    ///
    /// `tile_size` is the world-space side length of one rendered tile.
    pub fn new(ctx: &mut Context, tileset: &TileSet, map: &TileMap, tile_size: f32) -> Self {
        ...
        let layers = map
            .layers
            .iter()
            .map(|layer| build_layer_buffers(ctx, tileset, layer, tile_size))
            .collect();
    ...
    /// Rebuild GPU buffers after the map or tileset changes.
    pub fn rebuild(&mut self, ctx: &mut Context, tileset: &TileSet, map: &TileMap) {
        self.layers = map
            .layers
            .iter()
            .map(|layer| build_layer_buffers(ctx, tileset, layer, self.tile_size))
            .collect();
    }

    /// Upload the view-projection matrix. Call each frame before [`render`](Self::render).
    ...
    pub fn render(&self, pass: &mut RenderPass<'_>, tileset: &TileSet) {
        for layer_bufs in &self.layers {
            if layer_bufs.index_count == 0 {
                continue;
            }
            pass.set_pipeline(&self.pipeline);
            pass.set_uniform(0, &self.uniform);
            pass.set_texture(1, &tileset.texture);
            pass.set_vertex_buffer(0, &layer_bufs.vbuf);
            pass.draw_indexed_count(&layer_bufs.ibuf, layer_bufs.index_count);
        }
    }
}

/// Build GPU vertex/index buffers for a single layer.
fn build_layer_buffers(
    ctx: &mut Context,
    tileset: &TileSet,
    layer: &TileLayer,
    tile_size: f32,
    ...
    if layer.visible {
        for row in 0..layer.rows {
            for col in 0..layer.cols {
                let id = layer.get(col, row);
                let Some(uv) = tileset.uv(id) else { continue };

                if verts.len() / 4 >= MAX_TILES {
                    break;
                }

                let x0 = col as f32 * tile_size;
                // Negate Y so row 0 is at the top (Y-up camera convention).
                let y0 = -(row as f32 * tile_size);
                let x1 = x0 + tile_size;
                let y1 = y0 - tile_size;
    ...
    // wgpu buffers cannot be zero-sized; pad with one dummy entry if empty.
    if verts.is_empty() {
        verts.push(TileVert {
            pos: [0.0; 2],
            uv: [0.0; 2],
    ...
    let index_count = if layer.visible {
        indices.len() as u32
    } else {
        0
    };
