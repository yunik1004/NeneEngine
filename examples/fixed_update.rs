...
use nene::{
    math::{Mat4, Vec3, Vec4},
    renderer::{Context, IndexBuffer, Pipeline, PipelineDescriptor, RenderPass, UniformBuffer, VertexBuffer},
    time::{FixedTime, Time},
    uniform, vertex,
...
#[uniform] struct Mvp   { mat: Mat4 }
#[uniform] struct Color { rgba: Vec4 }
...
fn circle_mesh() -> (Vec<Vtx>, Vec<u32>) {
    ...
    for i in 0..=SIDES {
        ...
        verts.push(Vtx { pos: [a.cos(), a.sin()] });
    }
    let mut idx = Vec::new();
...
fn init(ctx: &mut Context) -> State {
    ...
    let gpu: Vec<BallGpu> = balls
        .iter()
        .map(|_| BallGpu {
            mvp_buf:   ctx.create_uniform_buffer(&Mvp   { mat:  Mat4::IDENTITY }),
...
fn fixed_update(
    ...
    for b in &mut state.balls {
        ...
        b.pos[1] += b.vel[1] * dt;
        // Bounce off walls
        if b.pos[0] + b.radius > HALF  { b.pos[0] = HALF  - b.radius; b.vel[0] = -b.vel[0].abs(); }
        if b.pos[0] - b.radius < -HALF { b.pos[0] = -HALF + b.radius; b.vel[0] =  b.vel[0].abs(); }
        if b.pos[1] + b.radius > HALF  { b.pos[1] = HALF  - b.radius; b.vel[1] = -b.vel[1].abs(); }
        if b.pos[1] - b.radius < -HALF { b.pos[1] = -HALF + b.radius; b.vel[1] =  b.vel[1].abs(); }
    }
}
