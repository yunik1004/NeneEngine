...
fn main() {
    ...
    println!("A4  mono:   {}Hz, {}ch, {} frames", a4.sample_rate(),  a4.channels(),  a4.sample_count());
    println!("A5  stereo: {}Hz, {}ch, {} frames", a5.sample_rate(),  a5.channels(),  a5.sample_count());
    println!("Blip mono:  {}Hz, {}ch, {} frames", blip.sample_rate(), blip.channels(), blip.sample_count());
    ...
    // ── 2. Half volume ────────────────────────────────────────────────────────
    println!("\n[2] A4 at 30% volume...");
    device.play_with(&a4, PlayOptions { volume: 0.30, ..Default::default() });
    sleep(1100);

    // ── 3. Pan hard left / hard right ─────────────────────────────────────────
    println!("\n[3] A4 panned hard LEFT...");
    device.play_with(&a4, PlayOptions { pan: -1.0, ..Default::default() });
    sleep(600);
    ...
    device.play_with(&a4, PlayOptions { pan: -1.0, ..Default::default() });
    device.play_with(&a5, PlayOptions { pan:  1.0, ..Default::default() });
    sleep(1200);

    // ── 5. Looping blip, stopped after 500 ms ────────────────────────────────
    println!("\n[5] Blip looping — stops in 500 ms...");
    let looping = device.play_with(&blip, PlayOptions { looping: true, volume: 0.7, ..Default::default() });
    sleep(500);
    looping.stop();
