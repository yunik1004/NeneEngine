...
fn model_cached_after_first_load() {
    ...
    let mut assets = Assets::new();
    let h1 = assets.model(&path).unwrap();
    let h2 = assets.model(&path).unwrap();
    assert!(
        h1 == h2,
...
fn different_paths_are_different_handles() {
    ...
    let mut assets = Assets::new();
    let ha = assets.model(&a).unwrap();
    let hb = assets.model(&b).unwrap();
    assert!(ha != hb);
}
...
fn evict_removes_only_target() {
    ...
    let mut assets = Assets::new();
    assets.model(&a).unwrap();
    assets.model(&b).unwrap();
...
fn handle_survives_cache_clear() {
    ...
    let handle = assets.model(&path).unwrap();
    assets.clear();
...
fn handle_clone_equals_original() {
    ...
    let mut assets = Assets::new();
    let h = assets.model(&path).unwrap();
    let h2 = h.clone();
    assert!(h == h2);
}
...
#[test]
fn model_missing_file_returns_error() {
    let mut assets = Assets::new();
    let result = assets.model("/nonexistent/path/file.obj");
