use nene::physics::d2::{BodyBuilder, BodyType, ColliderBuilder, World};
...
fn dynamic_body_falls_under_gravity() {
    ...
    let handle = world.add_body(BodyBuilder::dynamic().translation(0.0, 10.0));
...
fn fixed_body_does_not_move() {
    let mut world = World::new();
    let handle = world.add_body(BodyBuilder::fixed().translation(0.0, 0.0));
    for _ in 0..60 { world.step(); }
...
fn zero_gravity_body_stays_put() {
    ...
    let handle = world.add_body(BodyBuilder::dynamic().translation(3.0, 5.0));
    for _ in 0..60 { world.step(); }
    let pos = world.position(handle).unwrap();
...
fn step_dt_advances_simulation() {
    let mut world = World::new();
    let handle = world.add_body(BodyBuilder::dynamic().translation(0.0, 100.0));
    world.add_collider(ColliderBuilder::ball(0.5), handle);
